...
M=int(input()) # 컵의 위치를 바꾸는 횟수 (0<m<=50)
cupLoc=[1,2,3] # 공이 들어있는 컵의 번호는 첫번째 1번

for _ in range(M):
    first, second = map(int, input().split())
    
    firstI=cupLoc.index(first)
    secondI=cupLoc.index(second)
    
    cupLoc[firstI], cupLoc[secondI] = cupLoc[secondI], cupLoc[firstI]
    print(cupLoc)
    
print(cupLoc.index(1))